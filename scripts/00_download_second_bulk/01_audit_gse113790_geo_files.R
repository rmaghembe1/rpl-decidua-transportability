...
message("[audit] Auditing GSE113790 GEO file availability")

acc <- "GSE113790"
base_url <- "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE113nnn/GSE113790/suppl/"
...
index_file <- file.path(out_dir, "GSE113790_supplementary_index.html")
...
files <- files[
  grepl("\\.(txt|tsv|csv|gz|xlsx|xls|soft|tar|zip)$", file_name, ignore.case = TRUE) |
    grepl("GSE113790", file_name, ignore.case = TRUE)
]

fwrite(
  files,
  file.path(qc_dir, "GSE113790_supplementary_file_audit.tsv"),
  sep = "\t"
)
