...
message("[projection] Projecting GSE214607 signatures into GSE178535 bulk decidua matrix")

expr_file <- "results/qc/second_bulk_candidates/GSE178535_normalized_expression_log2.tsv"
meta_file <- "results/qc/second_bulk_candidates/GSE178535_sample_metadata_curated.tsv"
...
if (!file.exists(expr_file)) stop("[projection] Missing GSE178535 expression file: ", expr_file)
if (!file.exists(meta_file)) stop("[projection] Missing GSE178535 metadata file: ", meta_file)
...
if (any(is.na(meta$diagnosis))) {
  stop("[projection] Metadata contains unassigned diagnosis values. Fix metadata before projection.")
}
...
effects[, statistical_boundary := "Pilot only: GSE178535 is a small third bulk cohort; directional effect should not be interpreted as statistically confirmed"]

setorder(effects, -abs_delta)

fwrite(scores_long, file.path(out_dir, "GSE178535_GSE214607_projection_scores_long.tsv"), sep = "\t")
fwrite(scores_wide, file.path(out_dir, "GSE178535_GSE214607_projection_scores_wide.tsv"), sep = "\t")
fwrite(scores_annot, file.path(out_dir, "GSE178535_GSE214607_projection_scores_annotated.tsv"), sep = "\t")
fwrite(coverage, file.path(out_dir, "GSE178535_GSE214607_projection_signature_coverage.tsv"), sep = "\t")
fwrite(effects, file.path(transport_dir, "GSE178535_GSE214607_projection_transportability_summary.tsv"), sep = "\t")

message("[projection] Coverage:")
print(coverage[, .(signature_id, n_signature_genes, n_present, pct_present)])

message("[projection] GSE178535 group effects:")
print(effects[, .(
  signature_id,
  pct_present,
  mean_control,
