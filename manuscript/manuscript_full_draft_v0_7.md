...
> Citation status: v0.7 contains structured reference placeholders and corrected top-level section citation maps in the form `[REF:source_key]`, mapped to `manuscript/references/formal_citation_metadata_v0_1.tsv`. These placeholders should be converted to journal-formatted citations after target-journal selection.
...
Citation map: RPL clinical definition and heterogeneity should be supported by [REF:ESHRE_RPL_2022; REF:ASRM_RPL_2026]. Decidual and maternal-fetal-interface single-cell framing should be supported by [REF:VentoTormo_2018_Nature].
...
Citation map: The primary GSE214607 reference should be supported by [REF:GSE214607_GEO; REF:SRP400574_SRA; REF:Wei_2022_FrontImmunol]. External bulk decidua projection cohorts should be supported by [REF:GSE161969_GEO; REF:GSE161969_paper; REF:SRP293708_SRA; REF:GSE113790_GEO_or_OmicsDI; REF:SRP143517_SRA; REF:GSE178535_GEO_or_OmicsDI].
...
Citation map: Results should cite dataset sources only at first cohort mention, while primary numerical findings should point to repository result tables rather than external literature.
...
Citation map: Interpretation of maternal-fetal-interface biology should be supported by [REF:VentoTormo_2018_Nature], while clinical evidence-boundary statements should be supported by [REF:ESHRE_RPL_2022; REF:ASRM_RPL_2026]. Optional contextual discussion may use [REF:Trophoblast_decidual_crosstalk_2023] or [REF:Spatial_decidua_RPL_2025] only if directly relevant.
## Principal finding
